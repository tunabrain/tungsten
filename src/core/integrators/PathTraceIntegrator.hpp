...
public:
    PathTraceIntegrator(const PackedGeometry &mesh, const std::vector<std::shared_ptr<Light>> &lights)
    ...
    Vec3f traceSample(const Camera &cam, Vec2u pixel, SampleGenerator &sampler) final override
    {
        ...
        while (ray && bounce++ < MaxBounces) {
            ...
            /*uint32 c = hash32(ray.id0);
            return Vec3f(c >> 24, (c >> 16) & 0xFF, (c >> 8) & 0xFF)*1.0/256.0f;*/
