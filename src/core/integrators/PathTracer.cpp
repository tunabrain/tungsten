...
CONSTEXPR bool PathTracer::GeneralizedShadowRays;

PathTracer::PathTracer(TraceableScene *scene, const PathTraceSettings &settings, uint32 threadId)
: _scene(scene),
  _settings(settings),
  _threadId(threadId)
{
...
SurfaceScatterEvent PathTracer::makeLocalScatterEvent(IntersectionTemporary &data, IntersectionInfo &info,
...
bool PathTracer::isConsistent(const SurfaceScatterEvent &event, const Vec3f &w) const
{
...
Vec3f PathTracer::generalizedShadowRay(Ray &ray,
                           const Medium *medium,
...
Vec3f PathTracer::attenuatedEmission(const Primitive &light,
                         const Medium *medium,
                         float expectedDist,
                         IntersectionTemporary &data,
                         IntersectionInfo &info,
...
Vec3f PathTracer::lightSample(const TangentFrame &frame,
...
Vec3f PathTracer::bsdfSample(const TangentFrame &frame,
                     const Primitive &light,
                     const Bsdf &bsdf,
                     SurfaceScatterEvent &event,
                     const Medium *medium,
...
Vec3f PathTracer::volumeLightSample(VolumeScatterEvent &event,
                    const Primitive &light,
                    const Medium *medium,
                    bool performMis,
                    int bounce,
...
Vec3f PathTracer::volumePhaseSample(const Primitive &light,
...
Vec3f PathTracer::sampleDirect(const TangentFrame &frame,
                   const Primitive &light,
                   const Bsdf &bsdf,
                   SurfaceScatterEvent &event,
                   const Medium *medium,
...
Vec3f PathTracer::volumeSampleDirect(const Primitive &light,
                    VolumeScatterEvent &event,
                    const Medium *medium,
                    int bounce,
                    const Ray &parentRay)
...
const Primitive *PathTracer::chooseLight(SampleGenerator &sampler, const Vec3f &p, float &weight)
{
...
Vec3f PathTracer::volumeEstimateDirect(VolumeScatterEvent &event,
                    const Medium *medium,
                    int bounce,
                    const Ray &parentRay)
{
...
Vec3f PathTracer::estimateDirect(const TangentFrame &frame,
                     const Bsdf &bsdf,
                     SurfaceScatterEvent &event,
                     const Medium *medium,
                     int bounce,
...
bool PathTracer::handleVolume(SampleGenerator &sampler, UniformSampler &supplementalSampler,
...
bool PathTracer::handleSurface(IntersectionTemporary &data, IntersectionInfo &info,
