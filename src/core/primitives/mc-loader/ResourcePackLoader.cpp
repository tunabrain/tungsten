...
void ResourcePackLoader::buildBlockMapping()
{
    ...
    for (unsigned i = 0; i < document.Size(); ++i) {
        ...
        JsonUtils::fromJson(document[i], "opaque", opaque);
        JsonUtils::fromJson(document[i], "connects_fence", connectsFence);
        JsonUtils::fromJson(document[i], "connects_pane", connectsPane);
        JsonUtils::fromJson(document[i], "flammable", flammable);
...
void ResourcePackLoader::loadModels(File dir, std::string base)
{
