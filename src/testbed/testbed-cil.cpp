...
void computeQuantizationMatrix(const int N, int *qMatrix, const int Q)
{
    for (int y = 0; y < N; ++y) {
        for (int x = 0; x < N; ++x) {
            float fx = x*8.0f/N;
...
uint8 *encodeGrayscaleImage(uint8 *src, const int w, const int h, const int N, const int Q, int &bufSize)
{
    int blocksX = w/N;
    int blocksY = h/N;
    ...
    computeQuantizationMatrix(N, qMatrix, Q);
...
void decodeGrayscaleImage(uint8 *dst, const uint8 *src, const int bufSize, const int w, const int h, const int N, const int Q)
{
    int blocksX = w/N;
    int blocksY = h/N;
    int componentStride = blocksX*blocksY;
...
uint8 *encodeColorImage(uint8 *src, int w, int h, const int N, const int Q, const int subsample, int &bufSize)
{
    int chromaW = w/subsample;
    int chromaH = h/subsample;
        ...
        for (int x = 0; x < chromaW; ++x) {
            ...
            for (int i = 0; i < subsample; ++i) {
                for (int j = 0; j < subsample; ++j) {
                    cbDc += cbImg[(y*subsample + i)*w + x*subsample + j];
                    crDc += crImg[(y*subsample + i)*w + x*subsample + j];
                }
            }
            cbImg[x + y*chromaW] = cbDc/(subsample*subsample);
            crImg[x + y*chromaW] = crDc/(subsample*subsample);
        }
    }

    int blocksX = w/N, chromaBlocksX = chromaW/N;
    int blocksY = h/N, chromaBlocksY = chromaH/N;
    int componentStride = blocksX*blocksY, chromaComponentStride = chromaBlocksX*chromaBlocksY;
    ...
    int predictorBytes = (blocksX*blocksY + 3)/4;
    int chromaPredictorBytes = (chromaBlocksX*chromaBlocksY + 3)/4;
    int srcBytes = w*h;
    int chromaSrcBytes = chromaW*chromaH;
    int totalSize = predictorBytes + chromaPredictorBytes*2 + srcBytes + chromaSrcBytes*2;

    uint8 *srcBuf = new uint8[totalSize];
    uint8 *dstBuf = new uint8[totalSize];
    std::memset(srcBuf, 0, totalSize);

    uint8 *yPredictor = srcBuf;
    uint8 *cbPredictor = yPredictor + predictorBytes;
    uint8 *crPredictor = cbPredictor + chromaPredictorBytes;
    uint8 *yBlocks = srcBuf + predictorBytes + chromaPredictorBytes*2;
    uint8 *cbBlocks = yBlocks + srcBytes;
    uint8 *crBlocks = cbBlocks + chromaSrcBytes;

    blockEncodeImage( yPredictor,  yBlocks,  yImg, qMatrix, zigZag,       w,       h,       componentStride, N);
    blockEncodeImage(cbPredictor, cbBlocks, cbImg, qMatrix, zigZag, chromaW, chromaH, chromaComponentStride, N);
    blockEncodeImage(crPredictor, crBlocks, crImg, qMatrix, zigZag, chromaW, chromaH, chromaComponentStride, N);
...
void decodeColorImage(uint8 *dst, const uint8 *src, const int bufSize, const int w, const int h, const int N, const int Q, const int subsample)
{
    int chromaW = w/subsample;
    int chromaH = h/subsample;
    ...
    uint8 *yPredictor = dstBuf;
    uint8 *cbPredictor = yPredictor + predictorBytes;
    uint8 *crPredictor = cbPredictor + chromaPredictorBytes;
    uint8 *yBlocks = dstBuf + predictorBytes + chromaPredictorBytes*2;
    uint8 *cbBlocks = yBlocks + srcBytes;
    uint8 *crBlocks = cbBlocks + chromaSrcBytes;

    blockDecodeImage( yImg,  yPredictor,  yBlocks, qMatrix, zigZag,       w,       h,       componentStride, N);
    blockDecodeImage(cbImg, cbPredictor, cbBlocks, qMatrix, zigZag, chromaW, chromaH, chromaComponentStride, N);
    blockDecodeImage(crImg, crPredictor, crBlocks, qMatrix, zigZag, chromaW, chromaH, chromaComponentStride, N);

    for (int y = chromaH - 1; y >= 0; --y) {
        for (int x = chromaW - 1; x >= 0; --x) {
            uint8 cbDc = cbImg[x + y*chromaW];
            uint8 crDc = crImg[x + y*chromaW];

            for (int i = 0; i < subsample; ++i) {
                for (int j = 0; j < subsample; ++j) {
                    cbImg[(y*subsample + i)*w + x*subsample + j] = cbDc;
                    crImg[(y*subsample + i)*w + x*subsample + j] = crDc;
                }
            }
        }
    }
...
void testColorImage(const std::string &path)
{
    ...
    int w, h;
    uint8 *img = loadPaddedImage(w, h, path, 3, N*SubSample);
    ...
    if (img) {
        ...
        int bufSize;
        Timer timer;
        uint8 *compressed = encodeColorImage(img, w, h, N, Q, SubSample, bufSize);
        decodeColorImage(img, compressed, bufSize, w, h, N, Q, SubSample);
        timer.bench("Encode/decode round trip");
...
void testGrayscaleImage(const std::string &path)
{
    ...
    if (img) {
        ...
        int bufSize;
        uint8 *compressed = encodeGrayscaleImage(img, w, h, N, Q, bufSize);
        decodeGrayscaleImage(img, compressed, bufSize, w, h, N, Q);
...
//  idctTest();
//  transformTest();
//  testGrayscaleImage("C:/Users/Tuna brain/Desktop/TestImages/zone_plate.png");
//  testColorImage("C:/Users/Tuna brain/Desktop/CILTest/1.jpg");
    testColorImage("C:/Users/Tuna brain/Desktop/CIL/1.jpg");
//  testGrayscaleImage("C:/Users/Tuna brain/Desktop/CILTest/12.jpg");
    return 0;
}
